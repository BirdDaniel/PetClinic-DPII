...
class PerformanceH6Test extends Simulation {
	...
	setUp(addSnc.inject(atOnceUsers(1)),
		editSnc.inject(atOnceUsers(1)), 
		deleteSnc.inject(atOnceUsers(1)),
		noAddSnc.inject(atOnceUsers(1)),
		noEditSnc.inject(atOnceUsers(1)),
		noDeleteSnc.inject(atOnceUsers(1)),
		badURLSnc.inject(atOnceUsers(1))).protocols(httpProtocol)
}
